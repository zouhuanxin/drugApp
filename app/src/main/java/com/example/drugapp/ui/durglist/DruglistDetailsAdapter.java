...
public class DruglistDetailsAdapter extends RecyclerView.Adapter<DruglistDetailsAdapter.ViewHolder> {
    private List<Druglist2> list = new ArrayList<>();
    private DrugClick drugClick;

    public interface DrugClick {
        void Add(Druglist2 drug);
        void Delect(Druglist2 drug);
        void Update(Druglist2 drug);
    }

    public DruglistDetailsAdapter(List<Druglist2> list) {
        this.list = list;
    }
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_druglistdetails, parent, false);
        ...
        if (list.get(position) == null) {
            ...
            holder.adddrug.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    //add
                    if (drugClick != null) {
                        drugClick.Add(list.get(position));
                    }
                }
            });
        } else {
